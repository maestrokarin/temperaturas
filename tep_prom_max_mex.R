...
#Gráfica de línea por entidad federativa
temp%>%
  ggplot(., 
         aes(x = year, y = temp, group=entidad, color=entidad)) + 
  #Geom line con loess
  geom_line()+
  #Loess
  geom_smooth(method = "loess", se = F, size=1)+
  scale_x_continuous("Año",
                     breaks = seq(from = 1985, to = 2023, by = 2)
  )+
  scale_y_continuous("Temperatura máxima (°C)")+
  #Líneas rojas
  scale_color_manual(values = rep("red", 32))+
  theme_minimal()+
  ##Títulos, subtítulos y fuente
  labs(title = "Temperatura máxima promedio anual por entidad federativa",
       subtitle = "1985-2023",
       caption = "
Fuente: Elaborado por el Maestro Karin con datos de la Comisión Nacional del Agua (CONAGUA) datos.conagua.gob.mx")+
  ...
  facet_wrap(~entidad, ncol=4, scales="free_y")




##Salvar el gráfico
ggsave("temp_max_prom.png",height = 24,width = 30, units="in",dpi=300,bg="white")
